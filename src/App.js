		...
		initial: {
			on: {
				SUBMIT: [
					...
					{
						target: "error",
						actions: assign({ msg: (ctx, event) => "teste" })
					}
				]
			}
		},
...
const App = () => {
	...
	return (
		<div style={{ margin: "130px auto", width: "800px" }}>
			<MicroFlow machineValue={machine.value} />
...
const MicroFlowState = props => {
	return (
		<button
			className={`state btn ${
				props.machineValue === props.name ? "btn-danger" : "btn-primary"
			}`}
		>
			{props.name}
		</button>
	...
	const states = Object.keys(stateMachine.states).map(state => {
		return (
			<MicroFlowState
				name={state}
				key={state}
				machineValue={props.machineValue}
			/>
		);
	});
